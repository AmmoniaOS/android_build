...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^kos_") ; then
       KOS_BUILD=$(echo -n $1 | sed -e 's/^kos_//g')
    else
       KOS_BUILD=
    fi
    export KOS_BUILD
...
{
    target=$1
    KOS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/kos/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the KOS model name
            lunch kos_$target-userdebug
        fi
    fi
    return $?
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $KOS_FIXUP_COMMON_OUT ]; then
